...
static int smn_write_data(FILE *fp, unsigned int addr, unsigned int data)
{
	...
	ret = fseek(fp, SMN_ADDR_OFFSET, SEEK_SET);
	if (ret)
		return ret;
...
static char *get_rc_config_path(struct led_ctx *ctx, const char *path)
{
	...
	char *tmp;
	char *rc_config_path = NULL;
	char tmp_array[3] = {'\0'};
    /*
	 * For example, if this is the path to an SATA controller:
	 * "/sys/devices/pci0000:e0/0000:e0:07.2/0000:e2:00.1"
	 * we need to know the root complex the SATA controller attached,
	 * tmp is "pci0000:e0/0000:e0:07.2/0000:e2:00.1"
	*/
	...
	/* so the bus number is 0xe0 form the above example */
	...
	/* we can get the config path of root complex */
...
static char *get_nvme_config_path(struct led_ctx *ctx, const char *path)
{
	 ...
	 * we need to know the PCIe bridge's(NVMe controller attached) config file path,
	 * tmp is "/0000:81:00.0"
	*/
	tmp = strrchr(path, '/');
	len1 = strlen(tmp);
...
static int get_slot_control_reg_offset(struct led_ctx *ctx, FILE *fp)
{
	...
	read_pci_config_byte(fp, PCIE_CONFIG_STATUS_REGISTER_OFFSET,
				&pcie_status_reg);
	if (!(pcie_status_reg & PCIE_SUPORT_CAP_LIST)) {
		lib_log(ctx, LED_LOG_LEVEL_INFO, "[nvme]The PCIe not support cap list!\n");
		return -1;
	}
	read_pci_config_byte(fp, PCIE_CONFIG_CAP_POINTER_OFFSET, &capabilities_pointer);
	read_pci_config_byte(fp, capabilities_pointer, &capability_id);
	while((PCIE_CAP_STRUCTURE_ID != capability_id) && (capabilities_pointer != 0)) {
...
static int check_nvme_fw_config(struct led_ctx *ctx, const char *path)
{
	...
	if (!nvme_config_path) {
		lib_log(ctx, LED_LOG_LEVEL_INFO, "Get NVMe config path fail\n");
		goto fail0;
	}
...
static int check_sgpio_fw_config(struct led_ctx *ctx, const char *path, int host_nr)
{
	...
	if (!rc_config_path) {
		lib_log(ctx, LED_LOG_LEVEL_INFO, "Get rc config path fail\n");
		goto fail0;
	}
	...
	if (smn_read_data(fp, em_config_addr, &em_config_data) < 0)
		goto fail1;
	...
	if (smn_read_data(fp, em_loc_addr, &em_loc_data) < 0)
		goto fail2;
	if (em_loc_data == 0) {
		lib_log(ctx, LED_LOG_LEVEL_INFO,
...
static int send_sgpio_message(struct led_ctx *ctx, struct sgpio_message *msg)
{
	...
	if (smn_write_data(msg->fp, msg->sgpio_base + SGPIO_CFG, cfg_data) < 0)
		return -1;
	if (smn_write_data(msg->fp, msg->sgpio_base + SGPIO_REG_CNT, msg->reg_count) < 0)
		return -1;
	for (int i = 0; i < msg->reg_count; i++) {
		if (smn_write_data(msg->fp, msg->sgpio_base + SGPIO_WRITE_DATA + 4 * i,
			msg->buf[i]) < 0)
			return -1;
	}
	...
	if (smn_read_data(msg->fp, addr, &data) < 0)
		return -1;
	...
	return smn_write_data(msg->fp, addr, data);
}
							...
							enum led_ibpi_pattern ibpi)
{
	...
	msg->reg_index = 0x0;
	msg->reg_count = 0x2;
	msg->buf[0] = 0x00800000;
	msg->buf[1] = 0x0f0f7700;
	ret = send_sgpio_message(ctx, msg);
	if (ret < 0) {
